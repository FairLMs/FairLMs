...
:class:`~fairllms.datasets.FairnessDataset` or a sequence of dicts with
``stereotype`` / ``anti_stereotype`` keys); CAT takes
:class:`~fairllms.metrics.data.SentenceTriples`.
"""
...
from fairllms.metrics._compat import (
    as_examples,
...
from fairllms.metrics.data import SentenceTriples
...
class CrowSPairsScore(_PairMetric):
    ...
    optionally ``bias_type`` — see :class:`~fairllms.datasets.CrowSPairs`.
...
class ContextAssociationTestScore(FairnessMetric):
    ...
    ``data`` is a :class:`~fairllms.metrics.data.SentenceTriples`, or any
