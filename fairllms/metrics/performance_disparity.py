...
from fairllms.metrics.data import GroupProperties, QuerySpec, ScorePair
...
class AccuracyDisparity(FairnessMetric):
    ...
    ``data`` is a :class:`~fairllms.metrics.data.ScorePair` of per-item scores.
    No model is used — a plain-function equivalent lives in
    :mod:`fairllms.metrics.functional`.
    """
...
class BiasAmplifierScore(FairnessMetric):
    ...
    ``data`` is a :class:`~fairllms.metrics.data.GroupProperties`. Reports
...
class SensitiveNameSimilarity(FairnessMetric):
    ...
    ``data`` is a :class:`~fairllms.metrics.data.QuerySpec`. ``model`` must be a
