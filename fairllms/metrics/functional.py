...
>>> from fairllms.metrics.functional import equal_opportunity_gap
>>> equal_opportunity_gap([1, 1, 0], [1, 0, 0], ["A", "B", "A"], g1="A", g2="B")
0.5

Each returns a bare ``float`` (like ``accuracy_score``). Use the class form when
you want the full :class:`~fairllms.metrics.base.MetricResult` with diagnostics,
...
from fairllms.metrics.data import GroupPredictions, ScorePair
from fairllms.metrics.encoder_extrinsic import (
    ContextBasedDisparityScore,
    EqualOpportunityGap,
    FairInferenceScore,
)
from fairllms.metrics.encoder_decoder_extrinsic import InferenceBiasScore
from fairllms.metrics.performance_disparity import AccuracyDisparity
