...
from fairllms.metrics.data import ProbeSet, WordSets
...
class GradientBasedBiasEstimation(FairnessMetric):
    ...
    ``data`` is a :class:`~fairllms.metrics.data.WordSets`: ``target_1`` /
...
class NaturalIndirectEffect(FairnessMetric):
    ...
    ``data`` is a :class:`~fairllms.metrics.data.ProbeSet` whose probes each carry
