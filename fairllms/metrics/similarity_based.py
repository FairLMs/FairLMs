...
* ``__init__`` takes **keyword-only configuration**, stored verbatim, so
  :meth:`~fairllms.metrics.base.FairnessMetric.get_params` works and metrics can
  be cloned or swept.
* ``compute(model, data)`` takes exactly two positional arguments. Structured
  inputs arrive as validated containers from :mod:`fairllms.metrics.data`
...
from fairllms.metrics.data import ContextSets, VectorSets, WordSets
