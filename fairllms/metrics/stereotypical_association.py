...
from fairllms.metrics.data import ConceptSpec, OccupationTriples
...
class StereotypicalLogLikelihood(FairnessMetric):
    ...
    ``data`` is an :class:`~fairllms.metrics.data.OccupationTriples` of
...
class CooccurrenceAssociation(FairnessMetric):
    ...
    ``data`` is a :class:`~fairllms.metrics.data.ConceptSpec` carrying the
