...
`permutation_pval`) live in `fairLLMs.utils`.
...
| Effect size | Cohen's *d*: (mean(s(T1)) − mean(s(T2))) / pooled std (ddof=1) of all associations | `fairLLMs.utils` → `cohens_d()` |
| Association s(w, A, B) | mean cosine(w, A) − mean cosine(w, B) | `fairLLMs.utils` → `association_vectorized()` |
| p-value | One-sided permutation test over equal-size re-partitions of T1 ∪ T2. Exact enumeration when C(2n, n) ≤ `n_samples`, otherwise `n_samples` random permutations | `fairLLMs.utils` → `permutation_pval()` |
