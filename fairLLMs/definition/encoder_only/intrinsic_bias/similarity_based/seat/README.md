...
```python
from fairLLMs.metrics import SEAT
...
Sentence encoding lives in `fairLLMs.utils` →
...
| Pooling | `"mean"` — mask-aware mean over tokens | `main.py` / `fairLLMs.utils` |
| Per-term vector | the 5 template sentence embeddings are **averaged into one vector per word** | `seat.py` |
| Effect size | Cohen's d, pooled std (ddof=1) | `fairLLMs.utils` → `cohens_d()` |
| p-value | one-sided permutation test, `n_samples = 10000` | `fairLLMs.utils` → `permutation_pval()` |
