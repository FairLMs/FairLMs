...
```python
from fairLLMs.metrics import CEAT
...
python -m fairLLMs.definition.encoder_only.intrinsic_bias.similarity_based.ceat.main
```

Both scripts use local paths, so run them from inside this directory (the
package must also be importable; shared helpers live in `fairLLMs.utils`).
