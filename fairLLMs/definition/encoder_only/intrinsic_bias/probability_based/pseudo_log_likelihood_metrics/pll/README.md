...
| Dataset | Source |
|---|---|
| CrowS-Pairs (pooled) | `fairLLMs.datasets.CrowSPairs` / `fairLLMs/data/crows_pairs/` |
...
```bash
pip install -e .
python -m fairLLMs.definition.encoder_only.intrinsic_bias.probability_based.pseudo_log_likelihood_metrics.pll.main
```
