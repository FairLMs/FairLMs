"""Demo of ``CounterfactualFairnessScore`` with StereoSet / OpenML CF pairs (OpenAI-gated)."""

import os

from fairLLMs.definition.decoder_only.extrinsic_bias.counterfactual_fairness.ctf.stimuli import (
    load_german_credit,
    load_heart_disease,
    load_stereoset_gender_cf,
)
from fairLLMs.metrics import CounterfactualFairnessScore
from fairLLMs.models import OpenAIModel
...
def main():
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        print("OPENAI_API_KEY not set — stimuli restored; skipping CTF.compute()")
        if f_ss:
