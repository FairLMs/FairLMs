...
from ctf import compute_ctf, get_client, MODEL_NAME
...
def load_heart_disease(n_max: int = N_MAX) -> Tuple[List[str], List[str]]:
    ...
    for _, row in df.iterrows():
        ...
        if len(factual) >= n_max:
            break
    print(f"  Heart Disease: {len(factual)} pairs (CF = flip sex)")
    print(f"    [CAVEAT] sex is a REAL clinical risk factor -- a likelihood shift")
    print(f"             may reflect correct medical dependence, NOT unfairness.")
    return factual, counterfactual
...
def load_stereoset(n_max: int = N_MAX) -> Tuple[List[str], List[str]]:
    ...
    for bt, tgt, base in rows:
        ...
        if len(factual) >= n_max:
            break
    print(f"  StereoSet: {len(factual)} pairs (TARGET-GROUP swap, sentence fixed)")
    return factual, counterfactual


def run_dataset(client, name, factual, counterfactual, attr, caveat="") -> dict:
    n = min(len(factual), len(counterfactual), N_MAX)
    if n == 0:
        print(f"\n  [skip] {name} — no pairs")
        return None
    print(f"\n{'-' * 62}")
    print(f"  CTF  |  {name}  ({n} pairs)  |  attr: {attr}")
    print(f"{'-' * 62}")
    ctf, rows = compute_ctf(client, factual[:n], counterfactual[:n])
    print(f"  CTF = {ctf:.4f}   (0.0 = counterfactually fair; higher = larger shift)")
    return {"dataset": name, "ctf_score": ctf}


def main():
    print("\n" + "=" * 62)
    print(f"  Counterfactual Token Fairness (CTF)  |  {MODEL_NAME}")
    print("=" * 62)
    ...
    f, cf = load_heart_disease()
    r = run_dataset(client, "Heart Disease", f, cf, "sex")
    if r: results.append(r)
    ...
    out = results_to_csv(summary, "ctf_results.csv")
    print("\n" + "=" * 62)
    print("  CTF Summary  (lower = more counterfactually fair)")
    print("=" * 62)
    print(summary[["dataset", "ctf_score"]].to_string(index=False))
    print(f"\nSaved: {out}")
