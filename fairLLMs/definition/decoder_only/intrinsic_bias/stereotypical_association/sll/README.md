...
```python
from fairLLMs.metrics import StereotypicalLogLikelihood
...
| `data/` | Prefer `fairLLMs.datasets.BBQ` / bundled `fairLLMs/data/bbq/` |
...
```bash
pip install -e .
python -m fairLLMs.definition.decoder_only.intrinsic_bias.stereotypical_association.sll.main
```
