...
python -m fairlms.definition.encoder_only.intrinsic_bias.similarity_based.ceat.main
```

Both scripts use local paths, so run them from inside this directory (the
package must also be importable; shared helpers live in `fairlms.utils`).
