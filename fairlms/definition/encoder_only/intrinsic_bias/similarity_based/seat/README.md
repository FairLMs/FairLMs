...
Sentence encoding lives in `fairlms.utils` →
...
| Pooling | `"mean"` — mask-aware mean over tokens | `main.py` / `fairlms.utils` |
...
```bash
pip install -e .
python -m fairlms.definition.encoder_only.intrinsic_bias.similarity_based.seat.main
```
