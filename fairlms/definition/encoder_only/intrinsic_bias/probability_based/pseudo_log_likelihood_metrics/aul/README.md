...
Token scoring uses `score_sentence` in `fairlms.utils`.

## Datasets

| Dataset | Source |
|---|---|
| CrowS-Pairs (all bias types, pooled) | `fairlms.datasets.CrowSPairs` / `fairlms/data/crows_pairs/` |
...
```bash
pip install -e .
python -m fairlms.definition.encoder_only.intrinsic_bias.probability_based.pseudo_log_likelihood_metrics.aul.main
```
