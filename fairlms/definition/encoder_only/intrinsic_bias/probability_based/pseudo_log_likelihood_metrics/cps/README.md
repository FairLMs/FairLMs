...
sequences) lives in `fairlms.utils`.

## Datasets

| Dataset | Source |
|---|---|
| CrowS-Pairs (all 9 bias categories, pooled) | `fairlms.datasets.CrowSPairs` / `fairlms/data/crows_pairs/` |
...
| Token alignment | `difflib.SequenceMatcher` over token ids; positions marked `equal` are the shared span; `[CLS]`/`[SEP]` excluded | `cps.py`, `fairlms.utils` → `get_span()` |
