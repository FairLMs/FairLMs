...
```python
from fairlms.metrics import ContextBasedDisparityScore

# BBQ-style model outputs; prefer fairlms.datasets.BBQ + fairlms/data/bbq for inputs.
...
```bash
pip install -e .
python -m fairlms.definition.encoder_only.extrinsic_bias.context_based_disparity.main
```
