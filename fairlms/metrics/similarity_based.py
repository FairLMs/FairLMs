...
* ``__init__`` takes **keyword-only configuration**, stored verbatim, so
  :meth:`~fairlms.metrics.base.FairnessMetric.get_params` works and metrics can
  be cloned or swept.
* ``compute(model, data)`` takes exactly two positional arguments. Structured
  inputs arrive as validated containers from :mod:`fairlms.metrics.data`
...
from fairlms.definition.encoder_only.intrinsic_bias.similarity_based.ceat.ceat import (
    compute_ceat,
)
from fairlms.definition.encoder_only.intrinsic_bias.similarity_based.seat.seat import (
    compute_seat,
)
from fairlms.definition.encoder_only.intrinsic_bias.similarity_based.weat.weat import (
    compute_weat,
)
from fairlms.metrics.base import FairnessMetric, MetricResult
from fairlms.metrics.data import ContextSets, VectorSets, WordSets
from fairlms.metrics.resolve import get_tokenizer_model
from fairlms.utils import encode_sentence
