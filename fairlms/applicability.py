...
:func:`fairlms.metrics.resolve.check_task`, which established the pattern.

This module lives at the package root rather than inside
:mod:`fairlms.mitigation` because :mod:`fairlms.metrics` matches through it too,
and metrics must not import the mitigation layer.
"""
...
#: Model families, matching :attr:`fairlms.metrics.FairnessMetric.architectures`.
...
class ApplicabilityError(TypeError):
    ...
    raised by :func:`fairlms.metrics.resolve.check_task`: callers that guard a
